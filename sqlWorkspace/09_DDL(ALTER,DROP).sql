...
-- 1) 칼럼 추가/수정/삭제
--  1-1) 칼럼 추가(ADD) : ALTER TALBE 테이블명 ADD 칼럼명 테이터타입 [DEFAULT 기본값];
-- CNAME 칼럼을 테이블 맨 뒤에 추가한다.
-- 기본값을 지정하지 않으면 새로 추가된 칼럼은 NULL값으로 채워진다.

--  1-2) 칼럼 수정(MODIFY)
--    데이터 타입 변경 : ALTER TABLE 테이블명 MODIFY 칼럼명 변경할 데이터타입;
--    기본값 변경 : ALTER TABLE 테이블명 MODIFY 칼럼명 DEFALUT 변경할 기본값;
