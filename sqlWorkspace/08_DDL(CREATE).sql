...
DROP TABLE MEMBER; 
CREATE TABLE MEMBER(
    MEM_NO          NUMBER PRIMARY KEY -- UNIQUE + NOT NULL
  , ID              VARCHAR2(100)   CONSTRAINT MEM_ID_NN_ZZZZZ NOT NULL
  , PWD             VARCHAR2(100)   
  , NICK            VARCHAR2(100)
  , QUIT_YN         CHAR(1)         DEFAULT 'N' CONSTRAINT YN말고다른값넣지말라고ㅋㅋ CHECK( QUIT_YN IN('Y','N') )
  , ENROLL_DATE     TIMESTAMP
  , CONSTRAINT MEM_NICK_UQ_ABCDE UNIQUE(NICK)
);

INSERT INTO MEMBER(MEM_NO , ID, NICK) VALUES(NULL, 'USER02' , 'NICK02');
...
COMMENT ON COLUMN MEMBER.ID IS '회원의 아이디 칼럼 입니다~!';
COMMENT ON COLUMN MEMBER.PWD IS '비번';
COMMENT ON COLUMN MEMBER.NICK IS '회원 닉네임ㅋㅋ';
COMMENT ON COLUMN MEMBER.QUIT_YN IS 'Y:탈퇴한회원 , N:일반회원';
...
INSERT INTO MEMBER(ID,PWD,NICK,QUIT_YN) 
VALUES('USER01' , '1234', NULL ,'X');
