...
SELECT EMP_NAME, EMP_NO , SUBSTR(EMP_NO , 8 , 1) 성별코드
FROM EMPLOYEE
;
...
-- (991212-2****** 형태로 출력)
SELECT EMP_NAME, EMP_NO , SUBSTR(EMP_NO , 1, 8) || '******' AS 주민등록번호
FROM EMPLOYEE
;
SELECT EMP_NAME, EMP_NO , RPAD(SUBSTR(EMP_NO , 1, 8) , 14 , '*') AS 주민등록번호
FROM EMPLOYEE
;
...
-- 사원명, 직급코드 , 직급명 조회 (직급명은 아무렇게나 작성해도 됩니다)
SELECT 
    EMP_NAME 
    , JOB_CODE
    , DECODE(
...
-- 사원명 , 월급 , 구분(초급/중급/고급/특급)
--(급여300만원 까지는 초급 , 500만원 까지는 중급, 700만원 까지는 고급, 그 외에는 특급)
SELECT 
    EMP_NAME
    , SALARY
    , CASE
...
--        WHEN EMP_NO LIKE '_______1%' THEN '남자'
--        WHEN EMP_NO LIKE '_______2%' THEN '여자'
        ELSE '외국인'
    END AS 성별
FROM EMPLOYEE
ORDER BY SALARY DESC
